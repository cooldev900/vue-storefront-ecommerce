...
const mergeActions = {
  async updateClientItem ({ getters, rootGetters, dispatch }, { clientItem, serverItem }) {
    ...
    const currentFittingProduct = fittingProducts.find(p => p.sku === serverItem.sku)

    if (currentFittingProduct) {
      await VehicleStorage.removeFittingProduct(currentFittingProduct.sku)
      await VehicleStorage.setFittingProducts({
        ...currentFittingProduct,
        item_id: serverItem.item_id
      });

      await axios.post(
        `${config.api.url}/api/ext/fitting-service/add-fitting-service/${getters.getCartToken}/${serverItem.item_id}?addfitting=true&value=${currentFittingProduct.price}`
      );
    }
  ...
  async updateServerItem ({ getters, rootGetters, commit, dispatch }, { clientItem, serverItem, updateIds, mergeQty }) {
    ...
    const fittingPrice = rootGetters['vehicles/fittingProduct'] ? rootGetters['vehicles/fittingProduct'] : 0;
    const event = await CartService.updateItem(getters.getCartToken, cartItem, fittingPrice)
