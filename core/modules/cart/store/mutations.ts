...
const mutations: MutationTree<CartState> = {
  ...
  [types.CART_ADD_ITEM] (state, { product }) {
    ...
    if (!record) {
      ...
      if (record.groupedParents?.length && product.groupedParents?.length) {
        const newGroupedParents = record.groupedParents.filter(
          p => p.name !== product.groupedParents[0]['name']
        );

        EventBus.$emit('cart-before-update', {
          product: {
            ...record,
            groupedParents: [...newGroupedParents, ...product.groupedParents]
          }
        });
        record.groupedParents = [
          ...newGroupedParents,
          ...product.groupedParents
        ];
      } else {
