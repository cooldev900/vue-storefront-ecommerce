...
export function getProductPrice (product, customOptions = {}, fittingPrice = 0) {
  if (!product) {
    return {
      regular: '',
      special: ''
  ...
  return {
    ...
    special: isSpecialPrice ? formatPrice(special + fittingPrice) : ''
  };
}
...
export function getProductPriceFromTotals (product, fittingPrice = 0) {
  ...
  return {
    regular: formatPrice(regular + fittingPrice),
    special: isSpecialPrice ? formatPrice(special + fittingPrice) : ''
  };
}
