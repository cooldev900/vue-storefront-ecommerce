...
export const getters: GetterTree<PaypalState, any> = {
  ...
  getPlatformTotalSegments: (state, getters, rootState, rootGetters) =>
    rootState.cart.platformTotalSegments,

  getShippingDetails: (state, getters, rootState, rootGetters) =>
    rootState.checkout.shippingDetails,

  getPaymentDetails: (state, getters, rootState, rootGetters) =>
    rootState.checkout.paymentDetails,
  ...
  getAmount: (state, getters, rootState, rootGetters) => {
    const getSegmentTotal = (name) => {
      ...
      return total.length > 0
        ? Math.abs(parseFloat(total[0].value.toFixed(2)))
        : 0;
    };
    const currencyCode = rootState.storeView.i18n.currencyCode;
    const taxTotal = config.tax.finalPriceIncludesTax
      ? 0
      : getSegmentTotal('tax');
  ...
  getProducts: (state, getters, rootState, rootGetters) => {
    return rootState.cart.cartItems.map((product) => {
      ...
      const productPrice = config.tax.finalPriceIncludesTax
